...
half4 fragForwardBaseInternalWithClipping (VertexOutputForwardBase i, float4 pivotPoint, float cutOffDistance, float isStartGame)
{
    UNITY_APPLY_DITHER_CROSSFADE(i.pos.xy);
    ...
    if(isStartGame)
    {
        float distance_x = abs(pivotPoint.x - i.posWorld.x);
        float distance_y = abs(pivotPoint.y - i.posWorld.y);
        float distance_z = abs(pivotPoint.z - i.posWorld.z);

        clip(cutOffDistance - distance_x);
        //clip(cutOffDistance - distance_y);
        clip(cutOffDistance - distance_z);    
    }
